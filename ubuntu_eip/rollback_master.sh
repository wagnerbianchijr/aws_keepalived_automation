...
install_proxysql() {
  if ! command -v proxysql >/dev/null 2>&1; then
    ...
    curl -SL -o /tmp/proxysql.deb "$latest_url"
...
main() {
  ...
  read -rp $'\e[1m=> Enter Subnet ID for project ENI: \e[0m'
  SUBNET_ID=${SUBNET_ID:-subnet-0bb1c79de3}

  read -rp $'\e[1m=> Enter Security Group ID for project ENI: \e[0m'
  SG_ID=${SG_ID:-sg-0a1234567890abcdef}

  read -rp $'\e[1m=> Enter Backup node private IP: \e[0m'
  PEER_IP=${PEER_IP:-172.10.100.50}

  read -rp $'\e[1m=> Enter instance interface name (e.g. ens5): \e[0m'
  IFACE=${IFACE:-ens5}

  read -rp $'\e[1m=> Enter AllocationId of existing EIP (leave blank to allocate): \e[0m'
  ALLOCATION_ID=${ALLOCATION_ID:-eipalloc-0a341ee469e4b7447}
