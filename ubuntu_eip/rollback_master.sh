...
# Version     : v1.1-lite
# Date        : 2025-09-16
# Description : Rollback only local resources created by setup_master.sh.
#               Leaves IAM role/profile and EIP allocation intact.
# =============================================================================
...
sudo systemctl disable proxysql || true
...
sudo apt-get -y remove --purge keepalived awscli proxysql || true
sudo apt-get -y autoremove || true

# -----------------------------------------------------------------------------
# 3. AWS: Disassociate the EIP (but do not release)
# -----------------------------------------------------------------------------
...
if [[ -n "$PROJECT_ENI" ]]; then
  ...
  if [[ "$ATTACHMENT_ID" != "None" && "$ATTACHMENT_ID" != "null" ]]; then
    ...
    aws ec2 detach-network-interface --region "$REGION" --attachment-id "$ATTACHMENT_ID" --force || true
    sleep 3
  fi
  log INFO "Deleting ENI $PROJECT_ENI"
  aws ec2 delete-network-interface --region "$REGION" --network-interface-id "$PROJECT_ENI" || true
else
